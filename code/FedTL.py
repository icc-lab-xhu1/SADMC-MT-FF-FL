...
def train_and_test_load(class_name,train_loader,test_loader,num_classes,length,PreviousModel):
    ...
    print('AUC:{:.3f}'.format(roc_auc_score(label, pre_label)))
    acc = correct / total   # 准确率
    pre = preright_f / (pre_f + 0.1) # 精确率
    recall = preright_f / all_f # 召回率
    f1_score = 2 * pre * recall / (pre + recall + 0.1) # f1 score
    if num_classes == 4:
        pre2 = preright_2 / (pre_2 + 0.1)  # 精确率
        recall2 = preright_2 / (all_2 + 0.1)  # 召回率
        f1_score2 = 2 * pre2 * recall2 / (pre2 + recall2 + 0.1)  # f1 score
        pre3 = preright_3 / (pre_3 + 0.1)  # 精确率
        recall3 = preright_3 / (all_3 + 0.1)  # 召回率
        f1_score3 = 2 * pre3 * recall3 / (pre3 + recall3 + 0.1)  # f1 score
        print('precision:{:.3f}, recall:{:.3f}, f1_score:{:.3f}'.format(pre2, recall2, f1_score2))
        print('precision:{:.3f}, recall:{:.3f}, f1_score:{:.3f}'.format(pre3, recall3, f1_score3))
    TP = preright_f
    FP = pre_f - preright_f
...
# names = ['train.csv', 'train.csv', 'train.csv']
...
for i in range(len(names)):
    ...
    if i == 0:
        ...
        y_true, y_pre, model_state = train_and_test_load(classname, train_loader, test_loader, num_classes, length,
                                                         model_state)
