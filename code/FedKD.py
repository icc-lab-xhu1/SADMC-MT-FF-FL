...
def train_and_test(class_name, train_loader, test_loader, num_classes, length):
    ...
    f.writelines('MCC:{:.3f}'.format(MCC))
    print('precision:{:.3f}, recall:{:.3f}, f1_score:{:.3f}'.format(pre, recall, f1_score))
    f.writelines('precision:{:.3f}, recall:{:.3f}, f1_score:{:.3f}'.format(pre, recall, f1_score))
    if num_classes == 4:
        ...
        f.writelines('macro:{:.3f}, micro:{:.3f}'.format(macro, micro))
    # f.writelines(classname+':'+'MCC:'+MCC+','+'macro:'+macro+','+'micro:'++micro)
...
def train_and_test_load(class_name, train_loader, test_loader, num_classes, length, FCNmodel, index):
    ...
    # PreviousModel = net.OrdinaryKDTeFCN(input_num, num_classes, length, parameterlist).to(device)  # 获得model1
    # parametelist = FCNmodel.getAvgParameter(15)
    # para2 = FCNmodel.LoadParameters(index[1])
    # PreviousModel = net.OrdinaryKDTeFCN(input_num,num_classes,length,parametelist).to(device)
    # PreviousModel = net.MLSTM(length, num_classes, parametelist)
    parameterlist = FCNmodel.LoadParameters(index[0])
    PreviousModel = net.OrdinaryKDTeFCN(input_num, num_classes, length, parameterlist).to(device)
    para2 = FCNmodel.LoadParameters(index[1])
    PreviousModel2 = net.OrdinaryKDTeFCN(input_num, num_classes, length, para2).to(device)
    ...
    for epoch in range(epoches):
        for images, labels in train_loader:
            ...
            premodelist2 = PreviousModel2(images)
            ...
            loss = 0.9 * loss_func(outputlist[-1], labels) + 0.1 * (
                        net.SquareLossEW(outputlist[0], premodelist[0]) + net.SquareLossEW(outputlist[1],
                                                                                           premodelist[1]) +
                        net.SquareLossEW(outputlist[2], premodelist[2]) + net.SquareLossEW(outputlist[3], premodelist[
                    3]) + net.SquareLossEW(outputlist[0], premodelist2[0]) + net.SquareLossEW(outputlist[1],
                                                                                              premodelist2[1]) +
                        net.SquareLossEW(outputlist[2], premodelist2[2]) + net.SquareLossEW(outputlist[3],
                                                                                            premodelist2[3]))
    ...
    f1_score = 2 * pre * recall / (pre + recall)  # f1 score
    if num_classes == 4:
        pre2 = (preright_2 / pre_2) + 0.001  # 精确率
        recall2 = (preright_2 / all_2) + 0.001  # 召回率
        f1_score2 = (2 * pre2 * recall2 / (pre2 + recall2)) + 0.001  # f1 score
        pre3 = (preright_3 / pre_3) + 0.001  # 精确率
        recall3 = (preright_3 / all_3) + 0.001  # 召回率
        f1_score3 = (2 * pre3 * recall3 / (pre3 + recall3)) + 0.001  # f1 score
        print('precision:{:.3f}, recall:{:.3f}, f1_score:{:.3f}'.format(pre2, recall2, f1_score2))
        print('precision:{:.3f}, recall:{:.3f}, f1_score:{:.3f}'.format(pre3, recall3, f1_score3))
    TP = preright_f
    FP = pre_f - preright_f
...
# names = ['carts.csv','catalogue.csv','front-end.csv','orders.csv','payment.csv','shipping.csv','user.csv']
# names = ['skab.csv', 'swat-2015-data.csv', 'machine-1-1_test.csv','machine-1-2_test.csv','machine-1-3_test.csv','machine-1-4_test.csv','file_1.csv','file_2.csv','carts.csv','catalogue.csv','front-end.csv','orders.csv','payment.csv','shipping.csv','user.csv']
...
for i in range(len(names)):
    ...
    if i == 0:
        ...
        #del model_state
         ...
         y_true, y_pre, model_stateL = train_and_test(classname,train_loader,test_loader,num_classes,length)
