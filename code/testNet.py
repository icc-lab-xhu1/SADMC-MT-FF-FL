...
class MLSTM(nn.Module):
    def __init__(self, ni, nf, fc_drop_p=0):
        ...
        self.ea = ExternalAttention(d_model=ni, S=32)
        # self.aft_full = AFT_FULL(d_model=ni, n=128)
        # self.ea2 = ExternalAttention(d_model=(ni-7+2*2)+1, S=64)
        # self.ea3 = ExternalAttention(d_model=(ni-7+2*2)+1, S=64)
    ...
    def forward(self, x):
        ...
        # x = x.squeeze()
        # print(x.shape)
        # x_all = torch.cat((x.squeeze(), x2), dim=1)
        # print(x_all.shape)
        x_out = self.fc(x2)
...
def train_and_test(class_name, train_loader, test_loader, num_classes, length):
    ...
    return y_true, y_pre, test, model_Tstate
...
for i in range(len(names)):
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    y_true, y_pre, test, model_state = train_and_test(classname,train_loader,test_loader,num_classes,length)